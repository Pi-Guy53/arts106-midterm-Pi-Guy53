...
public class Player_Audio : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float Tension = Vector3.Distance(transform.position, TensionTrigger.transform.position);
        ...
        if(Tension < 9)
        {
            TensionAmbience.SetActive(true);
        }

        if(Tension > 9)
        {
            TensionAmbience.SetActive(false);
        }
